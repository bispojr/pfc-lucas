...
\textbf{ARL} também foi usada no trabalho de \citeonline{rodrigues2013modelo}, onde investigou-se a viabilidade da aplicação de técnicas de modelagem de regressão linear para realizar inferências relativas ao desempenho de estudantes. Os resultados obtidos com o experimento confirmam que é viável, podendo afirmar que é possível estimar o desempenho de alunos baseado na quantidade de interações em ferramentas do tipo fórum de discussão. 
...
\begin{table}[]
\caption{Características em relação a esse projeto de pesquisa}
\label{tab:my-table}
\resizebox{\textwidth}{!}{%
\begin{tabular}{c|cl|cc|cl|cc}
 & \multicolumn{2}{c|}{Coleta de Dados} & \multicolumn{2}{c|}{Técnicas/Métodos} & \multicolumn{2}{c|}{Disciplinas} & \multicolumn{2}{c}{Contexto Geográfico} \\ \hline
\begin{tabular}[c]{@{}c@{}}Artigos/\\Características\end{tabular} & \textit{\begin{tabular}[c]{@{}c@{}}Ferramentas\\ou Notas\end{tabular}} & \multicolumn{1}{c|}{Kahoot!} & \begin{tabular}[c]{@{}c@{}}Machine Learning\\ ou WEKA\end{tabular} & \begin{tabular}[c]{@{}c@{}}Rede Neural\\ Artificial\end{tabular} & \begin{tabular}[c]{@{}c@{}}CS1 ou\\ 1º Período\end{tabular} & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Interface\\ Humano-Computador\end{tabular}} & Brasil & \begin{tabular}[c]{@{}c@{}}Outros\\ países\end{tabular} \\ \hline
\citeonline{ahadi2016early} & X &  & X &  & X &  &  & X \\
\citeonline{Castro-Wunsch2017} & X &  &  & X & X &  &  & X \\
\citeonline{gotardo2013prediccao} & X &  & X &  & X &  & X &  \\
\citeonline{DeBrito2014} & X &  & X &  & X &  & X &  \\
\hline
Esse trabalho &  & \multicolumn{1}{c|}{X} &  & X &  & \multicolumn{1}{c|}{X} & X & 
\end{tabular}%
}
\end{table}

\textbf{Coletas de Dados}
\begin{itemize} 
    \item \textit{Snapshots} de códigos-fonte gerados: Foi utilizada uma ferramenta que capturava trechos dos códigos-fonte produzido pelos alunos.
    \item Iteração dos alunos no ambiente \textit{Moodle}: Contabilizava o número, e quais eram, de iterações feitas pelos alunos no ambiente \textit{Moodle}. \textit{Moodle} é o acrônimo de "\textit{Modular Object-Oriented Dynamic Learning Environment}", um software livre, de apoio à aprendizagem, executado num ambiente virtual.
    \item Notas - fornecidos pela STI da Universidade - de ingresso e do primeiro período do curso: Notas obtidas pelos alunos ao ingressarem na Universidade e as notas obtidas nas disciplinas do primeiro período (\ref{sec:1p}). As notas foram fornecidas pela STI - SUPERINTENDÊNCIA DE TECNOLOGIA DA INFORMAÇÃO da UFPB - Universidade Federal da Paraíba (\ref{sec:BR}).
    \item Kahoot!: Plataforma de Aprendizado Baseado em jogos que será usada neste trabalho. Descrita em: \ref{sec:Kahoot!}.
\end{itemize}


\textbf{Técnica/Método de Análise dos Dados}


\begin{itemize}
    \item Ferramentas de \textit{Machine Learning}: Uso de ferramentas que implementam e/ou acoplam técnicas de \textit{Machine Learning} para análise dos dados. ML foi descrita na seção.
    \item Redes Neurais Artificiais: Técnica que será usada nesse trabalho. Foi decrita em  
    \item WEKA - Redes Bayesianas, Árvore de Decisão e RNA: Ferramenta que tem implementada algumas técnicas de aprendizado de máquina.
\end{itemize}

\textbf{Unidade Local de Análise - Disciplina}

\begin{itemize}
    \item Introdução a Progrmação/CS1 em Python e CS1 em Java: Disciplinas introdutórias em programação.
    \item Primeiro Período: Contemplam as disciplinas de: Cálculo Diferencial e Integral 1, Física Aplicada à Computação 1, Cálculo Vetorial e Geometria Analítica.
    \item Interface Humano-Computador: Disciplina onde será aplicada o estudo de caso deste projeto de pesquisa.
\end{itemize}

\textbf{Contexto Geográfico}

\begin{itemize}
    \item Brasil: As universidades que aplicaram foram:
        \begin{itemize}
            \item Universidade Federal da Paraíba
            \item Uma Universidade à Distância omitida
            \item E este trabalho, na Universidade Federal de Goiás.
    \end{itemize}
    \item Estados Unidos: Uma Universidade Norte-Americana de Pesquisa Intensiva omitida.
    \item Europa: University of Helsinki, Finland e Uma Universidade Europeia de Pesquisa Intensiva omitida.
    \item Austrália: University of Technology, Sydney.
\end{itemize}
...
\chapter{Metodologia}
Um estudo de caso é uma investigação empírica que investiga um fenômeno contemporâneo dentro de seu contexto da vida real, especialmente quando os limites entre o fenômeno e o contexto não estão claramente deﬁnidos \cite{yin2001planejamento}. Esta investigação enfrenta uma situação tecnicamente única em que haverá muito mais variáveis de interesse do que fonte de dados, e, como resultado, (1) baseia-se em fontes de evidências, com os dados precisando assimilar-se a trabalhos correlatos, e, como outro resultado, (2)beneﬁcia-se do desenvolvimento prévio de proposições teóricas para conduzir a coleta e análise de dados.

\section{Descrição do estudo de caso}
Este é um estudo de caso {\color{red} Descrever}, utilizando uma estratégia exploratória. A justiﬁcativa deste estudo reside no caso revelador que é o uso de Redes Neurais Artificiais na predição de desempenho acadêmico dos alunos em Educação de Computação no Brasil. Os estudos do levantamento bibliográﬁco concentram-se principalmente em países da América do Norte, Europa e Oceania, como descrito no \autoref{sec:TR}.
Segundo \citeonline{yin2001planejamento} para um estudo de caso são especialmente importantes cinco componentes de um projeto de pesquisa, que serão descritos a seguir.

\subsection{Questões do estudo}
Para nortear este estudo de caso, a questão principal levantada foi \textbf{Q1: "É possível prever, de forma satisfatória, o desempenho acadêmico dos alunos em Educação de Computação no contexto Brasileiro?"}

\subsection{Proposições}
\label{sec:HS}
A seguinte proposição foi estabelecida a fim de ser investigada. \textbf{R1: "Sim, é possível alcançar um nível satisfatório de precisão e acurácia da previsão de desemepnho acadêmico dos alunos."}
...
A coleta dos dados terá duas fontes: as respostas e o desempenho final dos alunos da turma de 2018.2 (D1) e as respostas e o desempenho final dos alunos da turma de 2019.2 (D2) da disciplina de Interface Humano-Computador. 
...
Para verificar se há ligação entre os dados coletados e a proposição estabelecida, tomaremos como métrica para "previsão satisfatória" os resultados de precisão e acurácia obtidos por trabalhos correlatos. Os dados coletados servirão como suporte para a proposição através da métrica adotada. 

\subsubsection{Precisão e Acurácia}
Dois conceitos são importantes de serem explicados, Acurácia e Precisão.
...
\begin{figure}
    ...
    \caption{Diferença entre Acurácia e Precisão. Disponível em: http://www.baseaerofoto.com.br/faq/}
    \label{fig:PrecAcur}
\end{figure}
